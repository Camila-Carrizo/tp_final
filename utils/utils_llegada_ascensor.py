...
def simular_llegada_ascensor(estado_anterior, parametros, reloj):
    ...
    direccion = "sube" if estado_anterior["direccion"] == "baja" else "baja"
    estado = definir_estado()
    proxima_llegada_ascensor = reloj + uniforme(0, parametros["viaje_min"], parametros["viaje_max"]) if estado == "en_movimiento" else estado_anterior["PROXIMA_LLEGADA_ASCENSOR"]
    proxima_llegada_pasajero = estado_anterior["PROXIMA_LLEGADA_PASAJERO"]
    espacio_disponible = parametros["capacidad"] - h
    fin_descenso = reloj + p * parametros["tiempo_descenso_d"] if estado == "esperando_descenso" else None
    fin_ascenso = reloj + p * parametros["tiempo_ascenso_d"] if estado == "esperando_ascenso" else None
    fin_espera = None
    inicio_detencion = reloj if estado != "en_movimiento" else None
    ...
    acumulador_permanencia = estado_anterior["acumulador_permanencia"] + reloj - estado_anterior["RELOJ"] if estado == "en_movimiento" else estado_anterior["acumulador_permanencia"]
...
def calcular_cola(estado, direccion, espacio_disponible, cola_baja, cola_sube):
    """
    Si el estado es esperando_ascenso, bajan de la cola de esa dirección
...
def definir_estado(p, espacio_disponible, cola_baja, cola_sube, direccion):
    """
